...
def GSNN_MST(TR,TAR,VL,TARV,TST,TART):
  ...
  indpV = np.where(TARV == 1)
  indnV = np.where(TARV == 0)
  ...
  slctnumV = round(min(len(indnV) / 300, len(indpV)))
  TR = np.concatenate((TR, VL))
  TAR = np.concatenate((TAR, TARV))
  reps = 30
  ...
  for j1 in range(reps):
      ...
      slct = np.concatenate((slcto, slctoV))
  ...
  for j1 in range(reps):
    ...
    tarov = idct(tarovd)
    TARb = np.concatenate((taro, tarov))
  ...
  fpr, tpr, _ = roc_curve(TARo, res)
  auctr.append(trapz(tpr, fpr))

  fprv, tprv, _ = roc_curve(TARV, resV)
  aucval.append(trapz(tprv, fprv))

  fprt, tprt, _ = roc_curve(TART, resT)
  auctst.append(trapz(tprt, fprt))
